...
function trace(ray, t_min, t_max, dubina){
  ...
  var local_color = background_color
  if (!intersection) 
      return local_color;
  ...
  if(has_light){ 
    ...
    for(light of lights){
      light_power = add(light_power, light.compute_light(view, normal.direction, closest_object.specular));
    }

    local_color = multiply_colors(light_power, closest_object.color);  
  }
  else
      local_color = closest_object.color;

  if (dubina <= 0 || closest_object.reflective <= 0) 
      return local_color;

  var r = view.reflect(normal.direction);
  reflected_color = trace(r, EPSILON, Infinity, dubina - 1);
  return add(multiply(1-closest_object.reflective, local_color), multiply(closest_object.reflective, reflected_color));
}

function start_ray_tracer(objects){
   for (var x = -canvas.width/2; x < canvas.width/2; x++) {
        for (var y = -canvas.height/2; y < canvas.height/2; y++) {
            var direction = rotate(camera_rotation, canvas_to_viewport([x, y]))
            var color = clamp(trace(new Ray(camera_position, direction), 1, Infinity, num_reflections));  
            put_pixel(x, y, color);
        }
    }
}
